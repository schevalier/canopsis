...
class Storage(DataBase):
    ...
    PROPERTY = 'property'  #: property key for sort/index dict values
    DIRECTION = 'direction'  #: direction key for sort/index dict values
    ...
    def indexes(self, value):
        ...
        :param value: indexes such as:
            ...
            - one dict with mandatory key ``property`` and one optional key
                ``direction``.
            - a list items respecting previous types.
    ...
    @staticmethod
    def _get_directed(data):
        """Convert a data to a directed list where items are tuple of (name,
            direction).

        :param data: data to convert into the storage structure.
        :type data: str, tuple, dict or list
        :return: depending on data:
            - str: [(data, Storage.ASC)]
            - tuple: (data, order): [(data, order)]
            - dict: [(data[property], data.get(direction, Storage.ASC))]
            - list: [self._get_directed(item)+]
        """

        result = []

        if isinstance(data, basestring):  # one value
            result = [(data, Storage.ASC)]
        elif isinstance(data, dict):  # one value by key
            direction = data.get(Storage.DIRECTION, Storage.ASC)
            if isinstance(direction, basestring):
                direction = getattr(Storage, direction.upper())
            result = [(data[Storage.PROPERTY], direction)]
        elif isinstance(data, tuple):  # one value with order
            direction = data[1]
            if isinstance(direction, basestring):
                direction = getattr(Storage, direction.upper())
            result = [(data[0], direction)]
        elif isinstance(data, list):  # not empty list of indexes
            for item in data:  # convert
                result += Storage._get_directed(item)

        return result

    def bool_compare_and_swap(self, _id, oldvalue, newvalue):
        """Performs an atomic compare_and_swap operation on database related
        to input _id.
    ...
    def val_compare_and_swap(self, _id, oldvalue, newvalue):
        """Performs an atomic val_compare_and_swap operation on database
        related to input _id, oldvalue and newvalue.
