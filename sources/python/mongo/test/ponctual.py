...
from canopsis.mongo.periodic import MongoPonctualStorage
from canopsis.timeserie.timewindow import TimeWindow


class PonctualStoreTest(TestCase):
    """Test the ponctual storage."""

    def setUp(self):
        # create a storage on test_store collections
        self.storage = MongoPonctualStorage(data_scope="test_store")
    ...
    def test_crud(self):
        ...
        for data_id in data_ids:
            ...
            # add documents
            self.storage.put(data_id=data_id, points=points)
        ...
        for data_id in data_ids:
            ...
            count = self.storage.count(data_id=data_id, timewindow=timewindow)
            self.assertEquals(count, points_count_in_timewindow)

            data = self.storage.get(data_id=data_id)
            self.assertEquals(len(data), len(points))
            self.assertEquals(data, sorted_points)

            data = self.storage.get(data_id=data_id, timewindow=timewindow)
            ...
            self.storage.remove(data_id=data_id, timewindow=timewindow)
            ...
            self.storage.remove(data_id=data_id)
