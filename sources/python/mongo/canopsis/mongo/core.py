...
class MongoDataBase(DataBase):
    ...
    def _get_backend(self, backend=None, *args, **kwargs):
        ...
        :param str backend: backend name. If None, self.backend is used.
        :return: mongo backend reference.
...
class MongoStorage(MongoDataBase, Storage):
    ...
    def get_elements(
        ...
        if skip:
            cursor.skip(skip)
        if sort is not None:
            sort = Storage._get_directed(sort)
            cursor.sort(sort)
    ...
    def _get_hint(self, query, cursor):
        ...
        :param dict query: mongo query.
        :param MongoCursor cursor: mongo cursor.
        """
    ...
    def _manage_query_error(self, result_query):
        ...
        :return: result_query if no error encountered. Else None.
        """
